...
```gradle.kts
tasks.register<mpern.sap.commerce.build.tasks.HybrisAntTask>("unitTests") {
    ...
    //we can also call other ant targets by prefixing the target name with `y`
    //because the plugin defines a task rule (see below)
    dependsOn(tasks.named("ybuild"))
}
```
...
```gradle.kts
tasks.named("ycustomtarget") { mustRunAfter("yall") }
```
