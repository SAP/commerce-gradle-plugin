...
public class CloudV2Plugin implements Plugin<Project> {
    ...
    private void configureDefaultDependencies(HybrisPluginExtension extension, Project project, Manifest manifest) {
        ...
        hybrisPlatform.defaultDependencies(dependencies -> {
            ...
            String group = "de.hybris.platform";
            if (manifest.isPreview()) {
                group = "de.hybris.platform.preview";
            }
            dependencies.add(project.getDependencies()
                    .create(String.format("%s:hybris-commerce-suite:%s@zip", group, version.getDependencyVersion())));
            manifest.extensionPacks.forEach(p -> {
                if (!p.artifact.isEmpty()) {
                    dependencies.add(project.getDependencies().create(p.artifact));
                } else {
                    Version parseVersion = Version.parseVersion(p.getEffectiveVersion());
                    String extPackgroup = "de.hybris.platform";
                    if (p.isPreview()) {
                        extPackgroup = "de.hybris.platform.preview";
                    }
                    dependencies.add(project.getDependencies().create(
                            String.format("%s:%s:%s@zip", extPackgroup, p.name, parseVersion.getDependencyVersion())));
                }
            });
        });
    }
